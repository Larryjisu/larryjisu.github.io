...
기업 프로필 생성 모델도 GPT, Claude와 Gemini를 이용해 동일한 기업의 프로필을 각각 작성한 뒤, Ko-SRoBERTa 임베딩과 피어 검색을 거쳤을 때 어떤 모델의 프로필이 더 적절한 후보를 찾아내는지 비교할 수 있었다. 하지만 약 한 달 반의 개발 기간 안에서 모델별로 기업 프로필을 반복 생성하고, 사람 기준의 피어 정답셋과 대조해 결과를 검수하기에는 시간과 비용의 제약이 컸다. 이번 프로젝트에서는 피어 탐색부터 재무정보 조회와 최종 선택까지 이어지는 업무 파이프라인을 실제 사용 가능한 형태로 구현하는 데 우선순위를 두었다. 모델 간 비교평가와 평가 데이터셋의 확장은 후속 과제로 남겼다.
