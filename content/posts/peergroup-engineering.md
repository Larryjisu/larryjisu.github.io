...
## AS-IS: 피어 선정에서 시작되는 비상장기업 가치평가

VC는 비상장기업에 투자한 뒤의 예상수익률을 계산하기 위해 미래의 Exit 가치를 추정해야 한다. 그러나 시장에서 실시간으로 거래되는 상장사와 달리 비상장기업에는 관찰 가능한 시장가격이 없다. 이때 현실적인 차선책은 사업과 업종이 유사한 상장기업의 가치를 참고하는 것이다. 결국 **비상장기업 가치평가의 출발점은 적절한 피어그룹을 선정하는 일**이다.

기존 피어 선정과 가치평가 과정에는 다음과 같은 비효율이 존재했다.

1. **후보 탐색의 경험 의존성**

   피어 후보의 범위가 심사역의 배경지식과 경험에 따라 달라지고, 익숙하지 않은 산업에서는 유의미한 기업을 놓칠 가능성이 있다.

2. **반복적인 수작업**

   후보별 사업보고서와 재무정보를 개별적으로 찾아 정리한 뒤, 서로 다른 기준의 숫자를 맞춰가며 PER·PSR을 비교해야 한다.

3. **범용 LLM의 신뢰성 한계**

   LLM을 활용하면 탐색은 빨라지지만 환각 가능성이 남는다. 따라서 최신 사업정보와 피어 선정 근거, 재무 수치를 다시 확인하는 과정이 뒤따른다.

## TO-BE: 탐색·검증·가치평가를 하나의 흐름으로

이 프로젝트는 세 문제를 각각 다음과 같이 해결했다.

1. **경험에 의존하던 후보 탐색을 전 상장사 대상 하이브리드 검색으로 확장했다.**

   비상장기업의 웹사이트나 IR 자료를 회사 정체성, 제품·서비스, 시장, 수익구조, 밸류체인, 기술의 6개 필드로 구조화한다. 이 가운데 핵심 사업 정보를 한국어 임베딩과 BM25로 함께 검색해, 의미가 유사한 기업과 정확한 산업 용어가 겹치는 기업을 한국 상장사 전체에서 발굴한다.

2. **반복적인 자료 수집과 멀티플 계산을 데이터 파이프라인으로 자동화했다.**

   OpenDART 재무정보와 시가총액 데이터를 수집·정규화하고, FY·TTM 등 동일한 기준으로 PER·PSR을 계산한다. 사용자는 후보별 사업 내용, 재무 실적, 시가총액과 멀티플을 한 화면에서 비교하고 실제 가치평가에 포함할 피어를 직접 선택할 수 있다.

3. **LLM의 역할과 검증된 데이터의 역할을 분리했다.**

   LLM은 비정형 사업정보를 구조화하는 데 사용하되, 웹 원문과 인용 근거를 서버에서 다시 확인한다. 검증을 통과한 프로필만 임베딩·검색에 사용하고, 피어 순위는 임베딩·BM25 검색으로, PER·PSR은 공시와 시가총액에 기반한 결정론적 계산으로 산출한다. 이를 통해 LLM이 기업이나 재무 수치를 임의로 만들어 결과에 섞이는 경로를 차단했다.

**그래서 대상 기업의 사업을 분석해 상장 피어 후보를 발굴하고, 사업 유사성과 재무 멀티플을 한 화면에서 비교하는 피어 밸류에이션 워크스페이스를 구축했다.**

겉으로 보면 검색 화면과 API로 이루어진 웹 서비스지만, 실제 구현의 중심은 화면이나 API 프레임워크가 아니다. 비상장사와 상장사를 같은 언어로 표현하는 프로필 계약, 의미와 용어를 함께 반영하는 검색 계약, 같은 세대의 프로필·임베딩·재무만 서비스하는 데이터 게시 계약이 핵심이다. 이 글에서는 프론트엔드와 백엔드가 이 계약을 어떻게 나눠 맡는지, 그리고 LLMOps·임베딩·유지보수를 어떤 엔지니어링 루프로 묶었는지 설명한다.
